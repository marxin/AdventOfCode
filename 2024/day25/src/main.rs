...
const AREA: usize = 5 * 7;

fn parse_key_door(s: &str) -> HashSet<Point> {
    let mut set = HashSet::new();
    for (y, line) in s.lines().enumerate() {
        for (x, c) in line.chars().enumerate() {
            if c == '#' {
                set.insert(Point(x as i64, y as i64));
            }
        }
    }
    set
...
fn main() {
    ...
    let entries = content.split("\n\n").map(parse_key_door).collect_vec();
    dbg!(&entries.len());
    let count = entries
        .iter()
        .cartesian_product(&entries)
        .filter(|(x, y)| x != y && x.intersection(y).count() == 0) // && x.union(y).count() == AREA)
        .count();
    dbg!(count / 2);
}
